...
export async function generateMetadata({ searchParams }) {
  ...
  // No params — base page
  if (!params?.q && !params?.season && !params?.category && !params?.budgetMin) {
    return BASE_META;
  }

  const seo = getParamSEO('/guide', params);

  if (!seo.index) {
    // Noindex — canonical to base
    return {
      ...BASE_META,
      alternates: { canonical: 'https://budgetyatra.online/guide' },
  ...
  // Index — self-canonical with unique title/description
  return {
...
const faqSchema = {
  ...
  mainEntity: [
    ...
    { '@type': 'Question', name: 'What is the best 7-day budget India itinerary?', acceptedAnswer: { '@type': 'Answer', text: 'Top options: Golden Triangle Delhi–Agra–Jaipur (₹12,000–₹18,000), Kerala Backwaters Kochi–Alleppey–Munnar (₹10,000–₹15,000), or Himachal Delhi–Manali–Kasol (₹13,000–₹19,000).' } },
    { '@type': 'Question', name: 'What is the per-day budget for India road trip?', acceptedAnswer: { '@type': 'Answer', text: 'Budget traveler: ₹1,200–₹1,800/day. Mid-range: ₹2,500–₹4,000/day. Fuel costs ₹3–₹5/km for bikes and ₹6–₹10/km for cars.' } },
  ],
};
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">India Budget Road Trip Planner</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">Search destinations based on your budget and preferred travel season</p>
        </div>
        <div className="bg-white rounded-xl shadow-lg p-8 mb-12 animate-pulse">
          <div className="space-y-6">
            <div className="h-12 bg-gray-200 rounded-lg"></div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[...Array(4)].map((_, i) => <div key={i} className="h-12 bg-gray-200 rounded-lg"></div>)}
            </div>
            <div className="flex flex-wrap gap-3">
              {[...Array(4)].map((_, i) => <div key={i} className="h-10 w-24 bg-gray-200 rounded-full"></div>)}
            </div>
