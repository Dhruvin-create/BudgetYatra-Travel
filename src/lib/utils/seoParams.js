...
// High-intent destination/travel queries -- index + self-canonical
const HIGH_INTENT_QUERIES = new Set([
  // Destinations
  'goa', 'rishikesh', 'jaipur', 'manali', 'kerala', 'varanasi',
  'ladakh', 'hampi', 'uttarakhand', 'himachal', 'rajasthan',
  'karnataka', 'haridwar', 'varkala', 'kochi', 'nubra', 'pangong', 'leh',
  'mussoorie', 'nainital', 'shimla', 'ooty', 'munnar', 'pushkar', 'udaipur',
  // Travel types
  'mountains', 'beach', 'heritage', 'adventure', 'nature',
  'backwaters', 'india', 'budget travel', 'itinerary',
  // Activities with strong travel intent
  'rafting', 'trekking', 'backpacking',
]);

// Low-intent queries -- noindex (generic, thin content)
const LOW_INTENT_QUERIES = new Set([
  'packing', 'safety', 'students', 'food', 'scooter', 'yoga',
  'forts', 'seafood', 'snow', 'ruins', 'ghats', 'sarnath',
  'ganga aarti', 'family trip', 'low season', 'varkala',
]);
...
export function getParamSEO(pathname, params) {
  ...
  // Season pages -- index, self-canonical (no other params)
  ...
  // Explicitly low-intent queries -- noindex
  if (q && LOW_INTENT_QUERIES.has(q)) {
    return {
      index: false,
  ...
  // Unknown/random queries -- noindex, canonical to base
  return {
    index: false,
    canonical: `${SITE_URL}${pathname}`,
  };
...
function getQueryTitle(q) {
  const map = {
    ...
    karnataka: 'Budget Travel Guide to Karnataka -- Hampi, Coorg & More | BudgetYatra',
    haridwar: 'Budget Travel Guide to Haridwar -- Ganga Aarti & Temples | BudgetYatra',
    uttarakhand: 'Budget Travel in Uttarakhand -- Rishikesh, Haridwar & Hills | BudgetYatra',
    himachal: 'Budget Travel in Himachal Pradesh -- Manali, Shimla & More | BudgetYatra',
    rajasthan: 'Budget Travel in Rajasthan -- Jaipur, Jodhpur & Forts | BudgetYatra',
    ...
    rafting: 'River Rafting in India on a Budget -- Rishikesh & More | BudgetYatra',
    backpacking: 'Budget Backpacking India -- Routes, Costs & Tips | BudgetYatra',
  };
  const title = map[q];
...
function getQueryDescription(q) {
  const map = {
    goa: 'Complete budget travel guide to Goa with cheap hotels, food costs, transport tips, and a 3-day itinerary under Rs.10,000.',
    ...
    manali: 'Budget Manali travel guide with trip cost under Rs.15,000, best time to visit, cheap hotels, and 4-day itinerary.',
    ...
    karnataka: 'Budget travel guide to Karnataka -- Hampi ruins, Coorg coffee estates, and Mysore heritage on a budget.',
    haridwar: 'Budget Haridwar travel guide with Ganga Aarti timings, cheap hotels, and Rishikesh day trip tips.',
    uttarakhand: 'Budget travel in Uttarakhand -- Rishikesh rafting, Haridwar temples, and hill station trips under Rs.10,000.',
    himachal: 'Budget travel in Himachal Pradesh -- Manali, Shimla, Kasol with real costs and itineraries.',
    rajasthan: 'Budget travel in Rajasthan -- Jaipur, Jodhpur, Jaisalmer with cheap hotels and fort entry costs.',
    ...
    rafting: 'River rafting in India on a budget -- Rishikesh rafting cost, best routes, and safety tips.',
    backpacking: 'Budget backpacking India guide -- best routes, cheap hostels, food costs, and travel tips.',
  };
